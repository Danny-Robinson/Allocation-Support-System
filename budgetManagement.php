...
try {
    ...
    //QUERY TO SELECT TYPE AND SESSION ID FOR EACH SESSION
    $stmtType = $conn->prepare("SELECT lr.Type, a.Session_ID 
    						FROM Allocations a 
    ...
    $stmtType->setFetchMode(PDO::FETCH_ASSOC);
    $stmtType->execute();
    ...
    //array of weekstart timeslots
    $wsArray = array('1','46','91','136','181','226','271','316','361','406','451','496','541','586','631','676','721','766','811');
    //arrays of weekstart timestamps
    $stamps[] = array();
    try{
    	$stampQ = "SELECT * FROM Week_Date";		//collect week commencing timestamps from the database
    	foreach ($conn->query($stampQ) as $row) {
    		$stamps[] = $row['Week_Com'];
    ...
    $tsArray = array('1' => $stamps[0],'46' => $stamps[1],'91' => $stamps[2], '136' => $stamps[3],'181' => $stamps[4],'226' => $stamps[5],'271' => $stamps[6],'316' => $stamps[7],'361' => $stamps[8],'406' => $stamps[9], '451' => $stamps[10], '496' => $stamps[11],'541' => $stamps[12],'586' => $stamps[13],'631' => $stamps[14],'676' => $stamps[15],'721' => $stamps[16],'766' => $stamps[17],'811' => $stamps[0]);
    ...
    //COLLECT EACH MONTH'S BUDGET FROM DB
    ...
    //DISPLAY BASIC BUDGET OVERVIEW FOR YEAR
    echo "<br>";
