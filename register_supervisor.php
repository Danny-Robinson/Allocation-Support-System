<?php
	...
	echo "You are registering a <strong>Supervisor</strong><br>";			
	...
	$location = 'register_supervisor.php';

	$acc_type = 4;
	
	//Define the output log file for debugging purposes
	$file = 'log_reg_supervisor.txt';
	...
	$insert_error=$_GET["insert_error"]; //the insert error is either "" (if this page is hit for the first time or no DB insertion errors occurred) or "The Supervisor <Supervisor's full name> already exists!" 
	...
	if (isset($_POST['btn_reg_submit'])) {
  		...
  		do {	
			...
			try {
				...
				// Check if the person is already in the Supervisors table in DB (do not allow duplicates)
				$person = "Supervisor";
				...
				//insert the new staff member to the Supervisors table
